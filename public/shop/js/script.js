...
document.addEventListener('DOMContentLoaded', function () {
    ...
    tabs.forEach(tab => {
        tab.addEventListener('click', function () {
            ...
            // Находим соответствующую панель и активируем ее
    ...
    const firstTab = document.querySelector('.tabs__btn.active');
    if (firstTab) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Добавляем обработчики для поля имени
    if (nameInput) {
    ...
    function validateName() {
        ...
        const nameRegex = /^[a-zA-Zа-яА-ЯёЁ\s'-]+$/; // Разрешаем буквы, пробелы, апострофы и дефисы
        ...
        if (!nameRegex.test(name)) {
            showError(nameError, 'Invalid characters in name');
            return false;
        }
    ...
    // Скрываем требования к паролю изначально
    ...
    // Проверяем, если поле пароля уже заполнено (например, при возврате на страницу)
    ...
    // Обработчики событий для поля пароля
    if (passwordInput) {
    ...
    // Функция показа требований к паролю
    ...
    // Функция скрытия требований к паролю
    ...
    async function validateEmail() {
        ...
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
            showError(emailError, 'Enter a correct email');
            return false;
        }
        ...
        try {
            ...
            showError(emailError, 'Email verification error');
            return false;
        }
    }
    ...
    function validatePassword() {
        ...
        // 1. Длина пароля (минимум 8 символов)
        if (password.length < 8) {
            isValid = false;
            reqLength.classList.add('not-valid');
        } else {
        ...
        if (!/[A-Z]/.test(password)) {
            isValid = false;
            reqUppercase.classList.add('not-valid');
        } else {
            reqUppercase.classList.add('valid');
        }

        // 3. Строчные буквы (a-z)
        if (!/[a-z]/.test(password)) {
            isValid = false;
            reqLowercase.classList.add('not-valid');
        } else {
        ...
        if (!/[0-9]/.test(password)) {
            isValid = false;
            reqNumber.classList.add('not-valid');
        } else {
            reqNumber.classList.add('valid');
        }

        // 5. Специальные символы (!@#$%^&*_+-=)
        if (!/[!@#$%^&*_+\-=]/.test(password)) {
            isValid = false;
            reqSpecial.classList.add('not-valid');
        } else {
            reqSpecial.classList.add('valid');
        }

        if (!isValid) {
            showError(passwordError, 'Correct the highlighted requirements');
        } else {
    ...
    function resetPasswordRequirements() {
        [reqLength, reqUppercase, reqLowercase, reqNumber, reqSpecial].forEach(el => {
            if (el) el.classList.remove('valid', 'not-valid');
        });
    }
        ...
        try {
            ...
            if (data.redirect) {
                ...
                Object.entries(data.errors).forEach(([field, messages]) => {
                    ...
                    if (errorElement) {
                        showError(errorElement, messages[0]);
                    }
                });
            }
        } catch (error) {
            console.error('Ошибка:', error);
            showError(passwordError, 'Error during registration');
        }
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function handleLoginErrors(errors) {
        if (errors.email && emailError) {
            showError(emailError, errors.email[0]);
...
// Показ и скрітие пароля

document.addEventListener('DOMContentLoaded', function() {
    const passwordInput = document.getElementById('password');
    const toggleButton = document.querySelector('.field__action');
    const iconVisible = document.getElementById('no_hidden');
    const iconHidden = document.getElementById('hidden');
    const passwordRequirements = document.querySelector('.password-requirements');

    // Инициализация - скрываем иконку "скрыть" и требования
    if (iconHidden) iconHidden.style.display = 'none';
    if (passwordRequirements) passwordRequirements.style.display = 'none';

    // Показываем требования при фокусе на пароле
    if (passwordInput) {
        ...
        passwordInput.addEventListener('blur', function() {
            if (passwordRequirements && !this.value) {
                passwordRequirements.style.display = 'none';
            }
        });
    }

    // Обработчик клика по кнопке глаза
    if (toggleButton) {
